...
public class EntryCanvas extends javax.microedition.lcdui.Canvas{
  ...
  private String s;
  boolean comment1,comment2,comment3;
  private byte lastKey = 0;

  public EntryCanvas(String s,EntryCanvasProperties properties,BitArrayContainer font,int spaceLengthInPixel,
  ...
  */
  ...
  public void paint(Graphics g){
    ...
    if(comment3){
      g.drawString("Star key for delete", 0, disFromTop,
                   Graphics.TOP | Graphics.LEFT);
      disFromTop += g.getFont().getHeight();
    }
    if(s!=null){
      g.drawString(s, 0, disFromTop, Graphics.TOP | Graphics.LEFT);
      disFromTop += g.getFont().getHeight();
    }
  ...
  /*this method calls when a key pressed,if that key pressed again in short time
   ...
   private void deleteLast() {
       ...
       if(type==first)
           text[text.length-2]=searchAndGet(text[text.length - 2], first, seprate);
       else if(type==middle)
           text[text.length-2]=searchAndGet(text[text.length - 2], middle, last);
     ...
     public void keyRepeated(int keyCode){
       ...
       if(properties.chars[keyCode-48][0].length==0) return;
       repeatNumber=repeatNumber%properties.chars[keyCode-48][0].length+1;
   }
   ...
   public void keyReleased(int keyCode){
       ...
       if((repeatNumber==1)&&(!isRepeated)){
           ...
           else if((text[text.length-1]==space)||(text[text.length-1]==enter)
                   ||(properties.isLast(text[text.length-1])))
               newChar=properties.getSeprate(keyCode,repeatNumber-1);
           else newChar=properties.getLast(keyCode,repeatNumber-1);
           ...
           if(!((newChar==space)||(newChar==enter))){
             if (text.length != 0) {
               ...
               if (lastType == seprate)
                 newText[len - 2] = searchAndGet(newText[len - 2], seprate, first);
               else if (lastType == last)
                 newText[len - 2] = searchAndGet(newText[len - 2], last, middle);
           ...
           if(text.length<2)
             text[0]=properties.getSeprate(keyCode,repeatNumber-1);
           else{
       ...
       start=lastPageStart;
       while(!wasLastLine) repaint();
       if((keyCode==Canvas.KEY_STAR)||(keyCode==Canvas.KEY_POUND))return;
       if(this.hasRepeatEvents()) {repeatNumber=1;isRepeated=false;}
             ...
             if(properties.chars[keyCode][0].length==0) return;
             repeatNumber =repeatNumber% properties.chars[keyCode
                 ][0].length + 1;
         }

   }

   private byte searchAndGet(byte ch,byte lastType,byte newType){
       byte[] group=properties.getGroup(ch,lastType);
