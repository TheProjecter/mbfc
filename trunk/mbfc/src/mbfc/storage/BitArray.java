...
public class BitArray {
    ...
    public BitArray(byte aRow, byte aCol) {
        row = aRow;
        col = aCol;
    ...
    public BitArray(byte[] byteArray, byte aRow, byte aCol) {
        row = aRow;
        col = aCol;
        setBytes(byteArray);
    }
    ...
    public int getBit(int r, int c) {
        try {
            ...
            System.out.println(e.toString() + " " + r + " " + c);
        }
        return (1);
    }
    ...
    public byte getRowsLength() {
        return (row);
    }

    public byte getColsLength() {
        return (col);
    }
    ...
    public BitArray getACopyWithIncreasedHeight() {
        BitArray temp = new BitArray((byte) (this.row + 1), this.col);
        int realCol = (col - 1) / 8 + 1;
        for (int i = 0; i < row; i++) {
            for (int j = 0; j < realCol; j++) {
    ...
    public void paintIt(Graphics g, int top, int left, Color color) {
        g.setColor(color);
